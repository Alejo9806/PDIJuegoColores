...
fondoFinal = cv2.imread('./image/fondo.png')

verde_bajo = np.array ([100,100,20]) #Umbral bajo de color verde
verde_alto=np.array ([125,255,255])#Umbral alto de color verde
...
def captura_color(color_bajo, color_alto,color,confirmacion,frame):
    ...
    if confirmacion==True:#Si se logran leer las capturas de la camara se confirmará
        ...
        mascaraColor = cv2.bitwise_and(frame,frame, mask=mascara)
...
def juego():
    ...
    while True:
        ...
        frame=cv2.flip(frame,1)#evita el efecto espejo
        x_verde=captura_color(verde_bajo, verde_alto,(255,0,0),check,frame)#Identifica objeto de color verde y retorna su componente en X
...
while True:
    ...
    x_roja=captura_color(roja_bajo, roja_alto,(0,0,255),check,frame)#Identifica objeto de color rojo y retorna su componente en X
...
def pantallaFinal(marcador,perdidas):
    ...
    perdidasFinal = perdidas

    print(marcadorFinal,perdidasFinal)
    if marcadorFinal==5:#Si se llega a 5 puntos a favor se carga la imagen de ganador y pide mostrar objeto rojo para salir
        fondo=ganador
    ...
    elif perdidasFinal==5:#Si se llega a 5 puntos en contra se carga la imagen de perdedor y pide mostrar objeto rojo para salir
        fondo=perdedor
        ...
        cv2.putText(fondo, 'Muestra objeto rojo para salir',(80,450),cv2.FONT_HERSHEY_DUPLEX, 0.8, (255,255,255), 2)
        cv2.putText(fondo, 'Muestra objeto verde para reiniciar',(80,400),cv2.FONT_HERSHEY_DUPLEX, 0.8, (255,255,255), 2)
    return fondo

fondoFinal = pantallaFinal(marcador,perdidas)
...
while True:#Se inicia otro ciclo para salir del juego, al igual que al comeinzo se busca un objeto rojo para salir
    ...
    key2=cv2.waitKey(1)#Condicion para vizualización de la ventana con la imagen de la camara y cierre de dicha ventana
    x_roja=captura_color(roja_bajo, roja_alto,(0,0,255),check,frame)#Identifica objeto de color rojo y retorna su componente en X
    x_azul=captura_color(verde_bajo, verde_alto,(255,0,0),check,frame)#Identifica objeto de color rojo y retorna su componente en X
    # print(x_roja)#Imprime la componente en X del objeto rojo encontrado
    if x_azul !=0:
